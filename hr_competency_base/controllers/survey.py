...
class SurveyController(SurveyController):
    # Survey start
    # Override to pass employee_id from /my_<EVAL>/<int:eval_id>
    @http.route(['/survey/start/<model("survey.survey"):survey>',
                 '/survey/start/<model("survey.survey"):survey>/<string:token>'],
                type='http', auth='public', website=True, methods=['POST', 'GET'])
    def start_survey(self, survey, token=None, employee_id=None, **post):
        employee_id = employee_id and int(employee_id) or False
        UserInput = request.env['survey.user_input']
        SudoEmployee = request.env['hr.employee'].sudo()

        ## Test mode
        #if token and token == "phantom":
        #    _logger.info("[survey] Phantom mode")
        #    vals = {'survey_id': survey.id, 'test_entry': True}

        #    if survey.evaluation_id:
        #        employee_user = SudoUsers.search([('
        #        vals['employee_user_id'] = employee_user_id
        #        vals['evaluator_user_id'] = request.env.user.id

        #    user_input = UserInput.create(vals)
        #    data = {'survey': survey, 'page': None, 'token': user_input.token}
        #    if survey.evaluation_id:
        #        employee_user = SudoUsers.browse(employee_user_id)
        #        data['employee_name'] = employee_user.name
        #        print(data)
        ...
        if not token:
            ...
            if not request.env.user._is_public():
                ...
                vals['email'] = request.env.user.email

            if survey.evaluation_id:
                if survey.evaluation_id.evaluation_type == 'return_demo':
                    employee = SudoEmployee.browse(employee_id)
                    vals['employee_user_id'] = employee.user_id.id
                    vals['evaluator_user_id'] = request.env.user.id
                elif survey.evaluation_id.evaluation_type == 'knowledge_test':
                    vals['employee_user_id'] = request.env.user.id
                    vals['evaluator_user_id'] = request.env.user.id
        ...
        if user_input.state == 'new':  # Intro page
            data = {'survey': survey, 'page': None, 'token': user_input.token}
            if survey.evaluation_id:
                data['employee_user_name'] = request.env.user.name
            return request.render('survey.survey_init', data)
        else:
